...
#### Issues fixed
- Assets: During asset compilation, if a texture is too big for current graphics profile, an error will be displayed ([#174](https://github.com/SiliconStudio/xenko/issues/174)).
- Graphics: VertexArrayObject were not properly cached in D3D renderer ([#170](https://github.com/SiliconStudio/xenko/issues/170)).
- iOS: Properly link native library in latest Xamarin 8.6.1+ ([#140](https://github.com/SiliconStudio/xenko/issues/140)).
- Studio: Changing Settings->External Tools->Default Text/Shared Editor resulted in a crash ([#137](https://github.com/SiliconStudio/xenko/issues/137)).
- Studio: Pressing the Fit on screen button in the sprite editor while no image was displayed resulted in a crash ([#179](https://github.com/SiliconStudio/xenko/issues/179)).
...
#### New Features
- General: Xenko runtime now works in 64bit on Windows Desktop & Windows Store.
...
- Graphics: GeometricPrimitive.Plane.New now takes separate X and Y tessellation ([PR #130 from Jaanus Varus](https://github.com/SiliconStudio/xenko/pull/130)).
- Engine: ModelComponent.DrawOrder is now an integer.
- Shaders: Effect files (.pdxfx) needs to be saved again (ctrl+S), so that the associated .cs file is properly regenerated by Xenko Visual Studio Custom Tool.

#### Enhancements
- Android: Enable sticky fullscreen immersion mode (Android 4.4+) and fullscreen low profile mode (Android 4.0+) ([PR #135 from Sean Fausett](https://github.com/SiliconStudio/xenko/pull/135)).
...
- Graphics: Depth Stencil creation was failing on graphics feature level 10.0 (and 9.1 on Intel) [#163](https://github.com/SiliconStudio/xenko/issues/163)).
...
- Shaders: Fix code gen bug not allowing the usage of generic resources type in `pdxsl` files (like `Texture2D<int>`) ([#128](https://github.com/SiliconStudio/xenko/issues/128)).
...
#### Issues fixed
- Build: Fixed build issues due to Asset Compiler ([#116](https://github.com/SiliconStudio/xenko/issues/116)).
- Samples: Fixed various samples that didn't build since recent changes in SpriteBatch.MeasureString ([#117](https://github.com/SiliconStudio/xenko/issues/117)).
- Studio: Fixed high CPU usage due to improper WPF refreshes ([#115](https://github.com/SiliconStudio/xenko/issues/115)).
...
#### New Features
- Build: We now use OSS (OpenSource Signing) so that you can fake-sign assemblies with the same keys as ours, and use your own compiled Xenko with the official editor ([#88](https://github.com/SiliconStudio/xenko/issues/88)).
- Engine: Unified 2D and 3D rendering: `SpriteRenderer` now works with custom matrices or camera.
- Graphics: Spot light shadow maps in deferred rendering ([#96](https://github.com/SiliconStudio/xenko/issues/96)).
...
- Engine: `SpriteRenderer` now uses the Projection and View matrices set in the pipeline ([#96](https://github.com/SiliconStudio/xenko/issues/96)).
...
- Website: [Xenko Forums](http://forums.xenko3d.net) has just been opened. Feel free to use it to discuss about Xenko, help each other, collaborate and show off what you did with Xenko!
...
- Core: EnumerableExtensions.LastIndexOf() wasn't working properly ([#62](https://github.com/SiliconStudio/xenko/issues/62)).
- Game: Properly support windows with height 0 when AllowUserResizing is true ([#65](https://github.com/SiliconStudio/xenko/issues/65)).
- Game: GameForm is created with a black background, to avoid initial flickering while Windows is being initialized ([#54](https://github.com/SiliconStudio/xenko/issues/54)).
- Input: Alt+F4 is now properly working on Windows Store/Phone platforms ([#74](https://github.com/SiliconStudio/xenko/issues/74)).
- Input: Properly maps all extended keyboard keys on Windows Store/Phone platforms ([#84](https://github.com/SiliconStudio/xenko/issues/84)).
- Input: Fix several crashes and bugs in GestureRecognizers and mouse button states.
- Misc: PCL can now be used in Windows Store/Phone platforms ([#72](https://github.com/SiliconStudio/xenko/issues/72)).
- Samples: SimpleDynamicTexture was using expected screen size instead of actual screen size, resulting in incorrect picking in fullscreen mode ([#75](https://github.com/SiliconStudio/xenko/issues/75)).
...
- Studio: Fix binding errors in the property grid ([#29](https://github.com/SiliconStudio/xenko/issues/29)).
...
- Graphics: RasterizerState and DepthStencilState constructors are now private to match other Graphics classes. static New() should be used instead ([#83](https://github.com/SiliconStudio/xenko/issues/83)).
...
- Game: MicroThreads unhandled exceptions are not ignored anymore ([#47](https://github.com/SiliconStudio/xenko/issues/47)).
- Studio: Improved the different grid views in the interface.
- Visual Studio Package: Syntax highlighting now adapts to dark theme colors ([#26](https://github.com/SiliconStudio/xenko/issues/26)).

#### Issues fixed
- Audio: Fix crash in DynamicSoundEffectInstance worker thread during Dispose ([#34](https://github.com/SiliconStudio/xenko/issues/34)).
...
- Studio: Windows resize was slow in some specific cases ([#46](https://github.com/SiliconStudio/xenko/issues/46)).
- UI: Opacity is new correctly taken into account when drawing background color of UI elements ([#43](https://github.com/SiliconStudio/xenko/issues/43)).
- UI: Fix rendering problems on the Button/ToggleButton's content when setting their background color.
- Visual Studio Package: Syntax highlighting was not working properly on VS2012 ([#45](https://github.com/SiliconStudio/xenko/issues/45)).
...
We are thrilled to announce that Xenko is going [open source on github](https://github.com/SiliconStudio/xenko)! This is an important step toward the **empowerment of game developers**. We hope that this will make you more confident in using Xenko. You will have also an unique opportunity to contribute to the project and see by yourself how Xenko engine is working.

More details at [http://xenko3d.net/blog/new-version-open-sourcing](http://xenko3d.net/blog/new-version-open-sourcing).
...
#### Issues fixed
- Assets: Orientation of meshes from the assimp importer is now correct ([#22](https://github.com/SiliconStudio/xenko/issues/22)).
...
- Studio: Unsupported assets are ignored when trying to be imported ([#42](https://github.com/SiliconStudio/xenko/issues/42)).
...
- iOS: GraphicsDevice.UpdateSubresource() was using wrong OpenTK overload on iOS ([#35](https://github.com/SiliconStudio/xenko/issues/35)).
- iOS: libcore.a has been recompiled with libc++, to make it compatible with iOS 7+ ([#37](https://github.com/SiliconStudio/xenko/issues/37)).
- Shaders: Remove culture-dependant shader instanciation ([#38](https://github.com/SiliconStudio/xenko/issues/38)).
...
#### New Features
- Studio: Cut/copy/paste assets and asset directories (also works between different instances of Xenko Studio).
...
#### Issues fixed
- iOS: Xenko is now compatible with Xamarin iOS 8.2+ (Xamarin 3.7+).
- Graphics: Allow SpriteBatch to use a custom effect ([#30](https://github.com/SiliconStudio/xenko/issues/30)).
- Graphics: Use only space character in all pdxfx/pdxsl shaders ([#27](https://github.com/SiliconStudio/xenko/issues/27)).
- Shaders: Fix SV_InstanceID type in ShaderBase ([#33](https://github.com/SiliconStudio/xenko/issues/33)).
...
- Engine: Change some keys names from LightingKeys.xxx to Lighting.xxx. Xamarin Studio in the Settings menu to open solutions ([#5](https://github.com/SiliconStudio/xenko/issues/5)) ([#14](https://github.com/SiliconStudio/xenko/issues/14)).
...
- Studio: Main window won't go outside of the screen even on small resolution ([#13](https://github.com/SiliconStudio/xenko/issues/13))
...
- Input: Support gamepads reconnected ([#20](https://github.com/SiliconStudio/xenko/issues/20))
- Input: Breaking changes: Input.KeyPressed, KeyReleased removed in favour of Input.KeyEvents ([#21](https://github.com/SiliconStudio/xenko/issues/21)).
- Input: Filter ALT, F10 keys to avoid blocking game ([#21](https://github.com/SiliconStudio/xenko/issues/21)).
- Shaders: Use light intensity in deferred lighting.
- Studio: Fix creating game or sample with spaces and dot in project name, and dots in namespace ([#16](https://github.com/SiliconStudio/xenko/issues/16)) ([#17](https://github.com/SiliconStudio/xenko/issues/17)).
- Studio: Fix settings saving when Xenko is installed on another drive that the system drive.
...
- Visual Studio Package: Ignore projects that are not C# (csproj) ([#18](https://github.com/SiliconStudio/xenko/issues/18)).
- Visual Studio Package: .pdxsl/.pdxfx code generator was not working without VSSDK installed ([#19](https://github.com/SiliconStudio/xenko/issues/19)).
...
- Studio: Xenko Studio launcher can be moved and minimized.
...
- Setup: Fix installation program and allow to change installation directory ([#2](https://github.com/SiliconStudio/xenko/issues/2))
...
- Studio: Xenko Studio crashed when adding an Effect.Name (or any string) parameter to an Effect Library asset.
...
- Visual Studio: When Xenko version is updated, Visual Studio plugin will now offer to update files so that IntelliSense can refresh its cache ([#10](https://github.com/SiliconStudio/xenko/issues/10)).
...
#### Enhancements
- General: Visual Studio Express 2013 can now open Xenko Projects (with some limitations) ([#4](https://github.com/SiliconStudio/xenko/issues/4))
- Engine: Change default Windows output type to Windows Application ([#8](https://github.com/SiliconStudio/xenko/issues/8)): By default, a console is only opened when there is a message, assemblies are created in debug and the debugger is not attached.
...
#### Issues fixed
- Build: Android project fails to build ([#3](https://github.com/SiliconStudio/xenko/issues/3))
- Studio: GameStudio was using 100% of one CPU core ([#1](https://github.com/SiliconStudio/xenko/issues/1))
- Studio: Splash Screen always on top ([#6](https://github.com/SiliconStudio/xenko/issues/6))
- Studio: Fix GameStudio crash when importing assets using the add files button ([#9](https://github.com/SiliconStudio/xenko/issues/9))
