...
namespace SiliconStudio.Quantum.Tests
{
    ...
    public class TestGraphNodePath
    {
        ...
        public void TestPushTarget()
        {
            ...
            var nodes = new IContentNode[] { rootNode, rootNode.TryGetChild(nameof(Class.ClassMember)), targetNode };
            Assert.NotNull(targetNode);
            Assert.True(path.IsValid);
            Assert.False(path.IsEmpty);
            AssertAreEqual(rootNode, path.RootNode);
        ...
        public void TestPushTargetAndMember()
        {
            ...
            var nodes = new IContentNode[] { rootNode, rootNode.TryGetChild(nameof(Class.ClassMember)), targetNode, intNode };
            Assert.NotNull(targetNode);
            Assert.NotNull(intNode);
            Assert.True(path.IsValid);
            Assert.False(path.IsEmpty);
        ...
        public void TestPushIndex()
        {
            ...
            var nodes = new IContentNode[] { rootNode, rootNode.TryGetChild(nameof(Class.ListMember)), targetNode };
            Assert.NotNull(targetNode);
            Assert.True(path.IsValid);
            Assert.False(path.IsEmpty);
            AssertAreEqual(rootNode, path.RootNode);
        ...
        public void TestPushIndexAndMember()
        {
            ...
            var targetNode = nodeContainer.GetNode(obj.ListMember[1]);
            var intNode = targetNode.TryGetChild(nameof(Class.IntMember));
            var nodes = new IContentNode[] { rootNode, rootNode.TryGetChild(nameof(Class.ListMember)), targetNode, intNode };
            Assert.NotNull(targetNode);
            Assert.NotNull(intNode);
            Assert.True(path.IsValid);
            Assert.False(path.IsEmpty);
