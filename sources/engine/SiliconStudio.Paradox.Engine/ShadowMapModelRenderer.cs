...
namespace SiliconStudio.Paradox
{
    ...
    /// </summary>
    public class ShadowMapModelRenderer : ModelRenderer
    {
        #region Constructor

        public ShadowMapModelRenderer(IServiceRegistry services, string effectName) : base(services, effectName)
        {
        }
        ...
        protected override void UpdateMeshes(RenderContext context, ref FastList<EffectMesh> meshes)
        {
            ...
            {
                if (!meshes[i].MeshData.CastShadows || (context.ActiveLayers & meshes[i].MeshData.Layer) == 0)
                {
