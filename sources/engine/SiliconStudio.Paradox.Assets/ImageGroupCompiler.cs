        ...
        where TImageData : ImageFragmentData, new()
    {
        ...
        protected override Task<ResultStatus> DoCommandOverride(ICommandContext commandContext)
        {
            ...
            foreach (var image in asset.GroupAsset.Images)
            {
                ...
                if (asset.GroupAsset.AtlasPackingEnabled)
                {
                    if (!regionDictionary.ContainsKey(image)) continue;
                    ...
                    if (image.Orientation == ImageOrientation.Rotated90)
                        newImage.Orientation = newImage.Orientation == ImageOrientation.AsIs ? ImageOrientation.Rotated90 : ImageOrientation.AsIs;
                ...
                {
                    newImage.Texture = new ContentReference<Texture2D>
                    {
                        Location = (asset.GroupAsset.AtlasPackingEnabled)
                            ? ImageGroupAsset.BuildTextureAtlasUrl(Url, regionDictionary[image].Item1)
                            : ImageToTextureKey[image]
                    };
                }
            ...
            ref Dictionary<TImageInfo, Tuple<int, RotatableRectangle>> regionDictionary)
        {
            ...
            using (var texTool = new TextureTool())
            {
                ...
                for (var i = 0; i < asset.GroupAsset.Images.Count; ++i)
                {
                    ...
                    if (image.TextureRegion.Height == 0)
                        continue;

                    canUseRotation &= image.Orientation == ImageOrientation.AsIs;
                ...
                var texturePacker = new TexturePacker
                {
                    ...
                    UseRotation = canUseRotation && asset.GroupAsset.AtlasUseRotation,
                ...
                for (var textureAtlasIndex = 0; textureAtlasIndex < texturePacker.TextureAtlases.Count; ++textureAtlasIndex)
                {
                    ...
                    foreach (var texture in textureAtlas.Textures)
                    {
                        ...
                        regionDictionary.Add(imageInfoDictionary[textureKey], new Tuple<int, RotatableRectangle>(textureAtlasIndex, texture.PackingRegion));
                    }
