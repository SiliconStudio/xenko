...
extern "C" {
	...
	namespace OpenAL
	{
		...
		DLL_EXPORT_API xnAudioSource* xnAudioSourceCreate(xnAudioListener* listener, int sampleRate, int maxNBuffers, npBool mono, npBool spatialized, npBool streamed, int flags)
		{
			(void)spatialized;
			(void)maxNBuffers;
